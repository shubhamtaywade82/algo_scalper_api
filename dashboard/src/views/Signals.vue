...
const { recentSignals } = inject('dashboardState')

const processedSignals = computed(() => {
  const data = recentSignals?.value
...
<template>
  <div class="flex flex-col gap-6">
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
          ...
          <tbody class="divide-y divide-white/5">
            <tr v-for="sig in processedSignals" :key="sig.id" class="hover:bg-white/[0.03] transition-colors group">
              <td class="p-6">
                ...
                <div class="flex items-center justify-end gap-3">
                  ...
                  <div class="flex gap-1">
                    <div v-for="i in 5" :key="i"
                      :class="['w-1.5 h-4 rounded-full', i <= sig.confidenceBars ? sig.confidenceClass : 'bg-white/5']">
                    </div>
                  </div>
                </div>
              </td>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-4">
      <div v-for="item in [{label: 'Success Rate', val: '64%'}, {label: 'Avg Confidence', val: '0.82'}, {label: 'Peak Period', val: '10:15 IST'}]" :key="item.label"
        class="glass p-6 rounded-2xl flex flex-col gap-1 border border-white/5">
