...
const signalVariantMap = {
  'text-emerald-400 bg-emerald-500/10 border-emerald-500/20': 'success',
  'text-rose-400 bg-rose-500/10 border-rose-500/20': 'danger',
  'text-gray-400 bg-gray-500/10 border-gray-500/20': 'outline',
}

const outcomeVariantMap = {
  'text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 rounded-full': 'success',
  'text-amber-400 bg-amber-500/10 border border-amber-500/20 rounded-full': 'warning',
  'text-gray-400 bg-gray-500/10 border border-gray-500/20 rounded-full': 'outline',
  'text-gray-600': 'outline',
}

function getSignalVariant(signalClass) {
  return signalVariantMap[signalClass] || 'outline'
}

function getOutcomeVariant(cls) {
  return outcomeVariantMap[cls] || 'outline'
}
...
export default function Signals() {
  ...
  const processedSignals = createMemo(() =>
    signals().map(sig => {
      ...
      return {
        ...
        signalClass: getSignalVariant(sig.direction),
  ...
  function directionVariant(direction) {
    ...
    if (dir.includes('bullish') || dir.includes('buy')) return 'success'
    if (dir.includes('bearish') || dir.includes('sell')) return 'danger'
    return 'outline'
  }
      ...
      <div class="flex flex-col gap-6 w-full lg:w-3/4">
        <div class="flex items-center justify-between px-2">
          ...
          <div class="flex items-center gap-3">
            ...
            <Badge
              variant={meta().trading_mode === 'live' ? 'danger' : 'info'}
        ...
        <div class="glass rounded-2xl border border-white/5 px-6 py-4">
          <div class="flex flex-wrap items-end gap-4">
            ...
            <div class="flex gap-2 ml-auto">
              <Button
                variant="default"
                class="!bg-cyan-700 hover:!bg-cyan-600 text-[10px] font-black uppercase tracking-widest rounded"
              ...
              <Button
                variant="outline"
                class="text-[10px] font-black uppercase tracking-widest rounded"
            ...
            <Table>
              <TableHeader>
                <TableRow class="bg-white/[0.02]">
                  ...
                  <TableHead class="p-4 text-[10px] font-black text-gray-500 uppercase tracking-widest border-b border-white/5">Strategy</TableHead>
                  <TableHead class="p-4 text-[10px] font-black text-gray-500 uppercase tracking-widest border-b border-white/5 min-w-[200px]">Entry / Reason</TableHead>
              ...
              <TableBody class="divide-y divide-white/5">
                <Show when={loading()}>
                  <TableRow>
                    <TableCell colspan="7" class="p-12 text-center text-gray-600 text-xs uppercase tracking-widest">Loading...</TableCell>
                  </TableRow>
                </Show>
                <Show when={!loading()}>
                  <For each={processedSignals()}>
                    {(sig) => (
                      <TableRow class="hover:bg-white/[0.03] transition-colors group">
                        ...
                        <TableCell class="p-4">
                          <Badge variant="outline" class="text-[10px] font-black uppercase tracking-wide">
                            {sig.displayStrategy}
                          </Badge>
                        </TableCell>
                        <TableCell class="p-4 max-w-xs">
                          <div class="flex flex-col gap-1.5">
                            <Badge
                              variant={getOutcomeVariant(sig.entryOutcomeCls)}
                              class="text-[9px] font-black uppercase tracking-widest"
                            >
                              {sig.entryOutcomeLabel}
                            </Badge>
                            <Show when={sig.entryDetail}>
                              <span class="text-[9px] text-gray-400 leading-snug break-words" title={sig.entryDetail}>
                                {sig.entryDetail}
                              </span>
                            </Show>
                          </div>
                        ...
                        <TableCell class="p-4 pr-6 text-right">
                          <div class="flex items-center justify-end gap-3">
                            ...
                            <div class="flex gap-1">
                              <For each={[1, 2, 3, 4, 5]}>
                                {(i) => (
                                  <div class={`w-1.5 h-4 rounded-full ${i <= sig.confidenceBars ? sig.confidenceClass : 'bg-white/5'}`} />
                                )}
                              </For>
                            </div>
                          </div>
                  ...
                  <Show when={processedSignals().length === 0}>
                    <TableRow>
                      <TableCell colspan="7" class="p-20 text-center">
                    ...
                    return (
                      <>
                        ...
                        <Button
                          ...
                          class="text-[10px] font-black !w-7 !h-7 !p-0"
                          onClick={() => goToPage(p)}
                        >
                          {p}
                        </Button>
