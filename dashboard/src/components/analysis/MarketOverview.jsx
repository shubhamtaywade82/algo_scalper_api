...
export default function MarketOverview(props) {
  ...
  const regime = createMemo(() => data()?.market_regime)
  const metrics = createMemo(() => regime()?.metrics)

  const validities = createMemo(() => {
    if (!data()) return []
    return [
      { label: 'SMC', v: data().smc_validity },
      { label: 'Regime', v: data().regime_validity },
      { label: 'AI', v: data().ai_validity }
    ].filter(x => x.v)
  })
  ...
  return (
    <div class="glass rounded-2xl p-6 glass-hover">
      <div class="flex items-center justify-between mb-4">
        ...
        <Show when={validities().length > 0}>
          <div class="flex items-center gap-3">
            <For each={validities()}>
              {(item) => (
                <div class="flex items-center gap-1.5" title={`${item.label}: cached ${validityLabel(item.v)}, TTL ${item.v.ttl_seconds}s`}>
