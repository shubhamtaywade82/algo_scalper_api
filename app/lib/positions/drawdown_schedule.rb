...
module Positions
  module DrawdownSchedule
    ...
    def allowed_upward_drawdown_pct(profit_pct, index_key: nil)
      ...
      # When profit >= profit_max, return exactly dd_end_pct (before floor)
    ...
    def reverse_dynamic_sl_pct(pnl_pct, seconds_below_entry: 0, atr_ratio: 1.0)
      ...
      # Thresholds should be ordered from lowest to highest so most severe penalty applies first
      if cfg2[:atr_penalty_thresholds].is_a?(Array)
        sorted_thresholds = cfg2[:atr_penalty_thresholds].sort_by { |r| r[:threshold].to_f }
        sorted_thresholds.each do |r|
