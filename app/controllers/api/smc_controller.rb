...
module Api
  # SMC context/decision for alerts, scanners, and debugging (GET /api/smc/decision).
  class SmcController < ApplicationController
    include Api::TokenAuthenticatable

    before_action :authenticate_smc_decision!
    ...
    def decision
      ...
      Rails.logger.error("[Api::SmcController] #{e.class} - #{e.message}")
      render json: { ok: false, error: 'internal_error' }, status: :internal_server_error
    end

    private

    def authenticate_smc_decision!
      if params[:ai].to_s == '1'
        authenticate_operator_token!
      else
        authenticate_dashboard_token!
      end
    end
    ...
    def find_instrument
      ...
      instrument = Instrument.resolve_index_by_sid_or_symbol(security_id: security_id, segment_code: segment, symbol_name: symbol_name)
      unless instrument
