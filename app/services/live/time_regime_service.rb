...
module Live
  ...
  #
  # Market Session Segmentation (Indian Indices):
  # S1: OPEN EXPANSION     → 09:15 – 09:45 (Delta + Gamma dominant)
  # S2: TREND CONTINUATION → 09:45 – 11:30 (Best zone - Delta stable, Theta low)
  # S3: CHOP / DECAY       → 11:30 – 13:45 (Theta dominant - danger zone)
  # S4: CLOSE / GAMMA      → 13:45 – 15:15 (Theta + IV crush - tight rules)
  #
  ...
  class TimeRegimeService
    ...
    # Global override times
    NO_NEW_TRADES_AFTER = Time.parse('14:50').strftime('%H:%M')
    MARKET_OPEN = Time.parse('09:15').strftime('%H:%M')
    MARKET_CLOSE = Time.parse('15:15').strftime('%H:%M')
    ...
    def allow_new_trades?(time: nil)
      ...
      # No new trades after 14:50 (unless exceptional conditions)
