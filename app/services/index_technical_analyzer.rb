...
class IndexTechnicalAnalyzer < ApplicationService
  ...
  def load_configuration
    ...
    ANALYSIS_STRATEGIES.each_value do |strategy_config|
  ...
  def compute_indicators(timeframes, days_back)
    ...
    begin
      ...
      error_info = DhanhqErrorHandler.handle_dhanhq_error(e, context: 'technical_analysis')
  ...
  def compute_fallback_indicators(timeframes, _days_back)
    ...
    # Ensure DhanhqErrorHandler is loaded (used by instrument.candles() -> intraday_ohlc())
    # Trigger autoloading by referencing it - this ensures it's available when instrument.candles() calls intraday_ohlc()
    _ = DhanhqErrorHandler if defined?(DhanhqErrorHandler)
    ...
    timeframes.each do |tf|
      ...
      # Ensure DhanhqErrorHandler is available for error handling
      # The error might have occurred in instrument.candles() which uses the error handler
      log_error("Failed to compute indicators for #{tf}min: #{e.class} - #{e.message}")
    end

    @indicators = indicators_data.presence
  end
