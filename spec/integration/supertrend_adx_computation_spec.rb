...
RSpec.describe 'Supertrend & ADX Computation Integration', :vcr, type: :integration do
  ...
  before do
    ...
    allow(instrument).to receive_messages(candle_series: candle_series, adx: 25.5)
  end
  ...
  describe 'Supertrend Indicator Integration' do
    context 'when computing Supertrend with default parameters' do
      ...
      it 'calculates adaptive multipliers' do
        ...
        expect(result[:adaptive_multipliers].all?(Numeric)).to be true
      end
  ...
  describe 'Trading Indicators Module Integration' do
    ...
    context 'when computing Supertrend' do
      it 'calculates Supertrend correctly' do
        ...
        expect(st[:trend]).to be_in(%i[bullish bearish])
      end
