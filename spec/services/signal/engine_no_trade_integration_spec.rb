...
RSpec.describe Signal::Engine, 'No-Trade Engine Integration' do
  ...
  before do
    ...
    allow(nifty_instrument).to receive(:expiry_list).and_return([Date.today + 7.days])
    allow(nifty_instrument).to receive(:fetch_option_chain).and_return({
      ce: { '25000' => { 'oi' => 1000, 'iv' => 15.0, 'bid' => 100.0, 'ask' => 101.0 } },
      pe: { '25000' => { 'oi' => 2000, 'iv' => 14.0, 'bid' => 90.0, 'ask' => 91.0 } }
    })
    ...
    allow(nifty_instrument).to receive(:adx).and_return(20.0)

    # Mock strike selection
    allow(Options::ChainAnalyzer).to receive(:pick_strikes).and_return([
      { symbol: 'NIFTY25000CE', security_id: '50074', segment: 'NSE_FNO', ltp: 100.0, lot_size: 75 }
    ])
    ...
    context 'when Phase 1 blocks the trade' do
      it 'does not proceed to signal generation' do
        ...
        allow(described_class).to receive(:quick_no_trade_precheck).and_return({
          allowed: false,
          score: 3,
          reasons: ['Avoid first 3 minutes', 'Low volatility: 10m range < 0.1%', 'IV too low'],
      ...
      it 'logs the blocking reasons' do
        ...
        expect(Rails.logger).to receive(:warn).with(match(/score=3\/11/))
    ...
    context 'when Phase 2 blocks the trade' do
      it 'does not proceed to EntryGuard' do
        allow(described_class).to receive(:validate_no_trade_conditions).and_return({
          allowed: false,
          score: 3,
          reasons: ['Weak trend: ADX < 15', 'DI overlap: no directional strength', 'No BOS in last 10m']
        })
