...
RSpec.describe OptionsBuying::StreamConsumer do
  ...
  describe '#process_message!' do
    ...
    it 'evaluates the stream tick for the owning index and acks the message' do
      allow(OptionsBuying::StrategyEngine).to receive(:evaluate!)
      consumer.send(:process_message!, 'opt1', '123-0', {
                      'ltp' => '120.0', 'oi' => '4500', 'volume' => '9000', 'ts' => '1700000000'
                    })

      expect(OptionsBuying::StrategyEngine).to have_received(:evaluate!).with(
        index_key: 'NIFTY', security_id: 'opt1', current_tick: tick
      )
      expect(OptionsBuying::StateStore).to have_received(:xack).with('opt1', '123-0')
    end
    ...
    it 'still acks (no redelivery loop) when the security maps to no index' do
      ...
      expect(OptionsBuying::StrategyEngine).not_to have_received(:evaluate!)
