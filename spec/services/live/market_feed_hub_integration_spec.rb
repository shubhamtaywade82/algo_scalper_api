...
RSpec.describe "Live::MarketFeedHub Integration" do
  ...
  before do
    ...
    allow(ws_client).to receive(:subscribe_one).and_return({ already_subscribed: false })
    allow(ws_client).to receive(:subscribe_many).and_return([])
  ...
  describe 'Tick processing Pipeline' do
    ...
    it 'correctly handles index ticks for institutional data' do
      ...
      expect(MarketData::MarketCache.ltp('NIFTY', is_index: true)).to eq(22150.75)
    end
