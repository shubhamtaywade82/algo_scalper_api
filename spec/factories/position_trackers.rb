...
FactoryBot.define do
  factory :position_tracker do
    ...
    trait :nifty_position do
      association :instrument, factory: [ :instrument, :nifty_future ]
      security_id { "12345" }
      segment { "derivatives" }
      quantity { 25 }
      avg_price { BigDecimal("25000.00") }
    ...
    trait :banknifty_position do
      association :instrument, factory: [ :instrument, :banknifty_future ]
      security_id { "67890" }
      segment { "derivatives" }
      quantity { 15 }
    ...
    trait :option_position do
      association :instrument, factory: [ :instrument, :nifty_call_option ]
      security_id { "11111" }
      segment { "derivatives" }
      quantity { 25 }
