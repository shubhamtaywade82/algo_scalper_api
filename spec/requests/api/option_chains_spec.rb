...
RSpec.describe 'Api::OptionChains' do
  describe 'GET /api/option_chain/:index_key' do
    ...
    before do
      ...
      allow(analyzer_double).to receive(:spot_ltp).and_return(24398.70)
      allow(analyzer_double).to receive(:find_nearest_expiry).and_return('2026-07-09')
      allow(analyzer_double).to receive(:load_chain_for_expiry).with('2026-07-09', 24398.70).and_return([
        {
          strike: 24400.0,
          type: 'CE',
          security_id: '12345',
    ...
    it 'returns option chain snapshot for valid index' do
      ...
      expect(body['spot']).to eq(24398.70)
      expect(body['atm_strike']).to eq(24400.0)
      expect(body['expiry']).to eq('2026-07-09')
      expect(body['legs'].size).to eq(2)

      ce = body['legs'].find { |l| l['type'] == 'CE' }
      expect(ce['strike']).to eq(24400.0)
