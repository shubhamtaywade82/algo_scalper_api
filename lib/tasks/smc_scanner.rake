...
namespace :smc do
  ...
  task :scan, [:index_key] => :environment do |_t, args|
    ...
    filtered_indices.each_with_index do |idx_cfg, index|
      ...
      if index.positive?
        Rails.logger.debug { "[SmcScanner] Waiting #{DELAY_BETWEEN_INSTRUMENTS}s before next instrument..." }
      ...
      unless instrument
        Rails.logger.warn("[SmcScanner] Instrument not found for #{idx_cfg[:key]} (#{idx_cfg[:segment]}/#{idx_cfg[:sid]})")
        next
      end
      ...
      begin
        ...
        Rails.logger.info("[SmcScanner] #{idx_cfg[:key]}: #{decision}")

        # If AI is enabled and we have a valid signal, get AI analysis
        if engine.ai_enabled? # && %i[call put].include?(decision)
          Rails.logger.info("[SmcScanner] Getting AI analysis for #{idx_cfg[:key]} #{decision} signal...")
          ai_analysis = engine.analyze_with_ai
          if ai_analysis.present?
            Rails.logger.info("[SmcScanner] AI Analysis for #{idx_cfg[:key]}:")
            ...
            Rails.logger.warn("[SmcScanner] AI analysis returned empty for #{idx_cfg[:key]}")
          end
        end
      rescue DhanHQ::RateLimitError => e
        Rails.logger.error("[SmcScanner] Rate limit error for #{idx_cfg[:key]}: #{e.message}")
        Rails.logger.info('[SmcScanner] Waiting 5 seconds before continuing...')
        sleep(5)
        next
      rescue StandardError => e
        Rails.logger.error("[SmcScanner] Error processing #{idx_cfg[:key]}: #{e.class} - #{e.message}")
        Rails.logger.debug { e.backtrace.first(5).join("\n") }
        next
      end
    end

    Rails.logger.info('[SmcScanner] Scan completed')
  end
  ...
  def filter_indices_by_expiry(indices)
    ...
    indices.each do |idx_cfg|
      ...
      unless instrument
        Rails.logger.warn("[SmcScanner] Instrument not found for #{idx_cfg[:key]} - skipping expiry check")
      ...
      if days_to_expiry > max_expiry_days
        Rails.logger.info(
          "[SmcScanner] Skipping #{idx_cfg[:key]} - expiry in #{days_to_expiry} days " \
    ...
    Rails.logger.error("[SmcScanner] Error filtering indices by expiry: #{e.class} - #{e.message}")
  ...
  def calculate_days_to_expiry(instrument)
    ...
    Rails.logger.warn("[SmcScanner] Error calculating expiry for #{instrument.symbol_name}: #{e.class} - #{e.message}")
  ...
  def send_ai_analysis_telegram_notification(index_key, decision, ai_analysis)
    ...
    begin
      ...
      Rails.logger.info("[SmcScanner] Sent AI analysis Telegram notification for #{index_key}")
    rescue StandardError => e
