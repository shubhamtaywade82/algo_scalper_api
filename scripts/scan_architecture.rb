...
def scan_guards
  ...
  if Dir.exist?(gd)
    Dir.glob(File.join(gd, '*.rb')).sort.each do |f|
...
def scan_risk_rules
  ...
  Dir.glob(File.join(rd, '*_rule.rb')).sort.map { |f| File.basename(f, '.rb') }
end
...
def scan_jobs
  ...
  require 'yaml'
  yaml = YAML.safe_load(File.read(yf))
...
def ts_esc(s); s.to_s.gsub("'", "\\'").gsub("\n", ' '); end

def ts_arr(items, &block)
  return '[]' unless items && items.any?
  "[\n" + items.map { |i| '  ' + block.call(i) }.join(",\n") + "\n]"
end
...
def generate_ts
  ...
  deltas = [
    ...
    direction = actual.is_a?(Numeric) ? (actual > review.to_i ? 'up' : actual < review.to_i ? 'down' : 'same') : 'up'
  ...
  <<~TYPESCRIPT
    ...
    export const aiSubsystems: AiSubsystem[] = #{ts_arr(ai) { |a| comps = a[:components].map { |c| "'#{ts_esc(c)}'" }.join(', '); "{\n      name: '#{ts_esc(a[:name])}',\n      path: '#{ts_esc(a[:path])}',\n      components: [#{comps}],\n      description: '#{ts_esc(a[:description])}',\n      status: '#{a[:status]}'\n    }" }};
    ...
    export const smcDetectors: string[] = #{ts_str_arr([
