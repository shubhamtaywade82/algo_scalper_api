...
PositionTracker.paper.order(created_at: :asc)
...
decline_period = timeline[peak_index..] if peak_index
...
if decline_period
  ...
  decline_period[1..].each_with_index do |event, idx|
...
if worst_streak[:count].positive?
...
losing_trades.sort_by(&:updated_at).each do |tracker|
...
puts '-' * 100
positions_with_hwm = exited_trackers.select do |t|
...
positions_with_hwm.each do |tracker|
  ...
  next unless drawdown > 1000 # Significant drawdown
