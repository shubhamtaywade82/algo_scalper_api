...
tracker = PositionTracker.active.first || create(:position_tracker,
                                                  order_no: 'TEST-SIM-001',
                                                  security_id: '49081',
                                                  segment: 'NSE_FNO', # Futures & Options segment
                                                  entry_price: 150.0,
                                                  quantity: 75,
                                                  status: 'active')
...
# Track exit calls for idempotency test
exit_calls = []
allow(exit_engine).to receive(:execute_exit) do |tracker_arg, reason_arg|
  exit_calls << { tracker: tracker_arg.id, reason: reason_arg, timestamp: Time.current }
  true
end
...
exit_calls.clear # Reset exit tracking
...
drawdown_sequence.each do |tick_data|
  ...
  if result[:exit_triggered]
    ...
    ServiceTestHelper.print_info("    Exit reason: #{exit_calls.last[:reason]}")
...
exit_calls.clear
exit_count_before = exit_calls.size
...
exit_count_after = exit_calls.size
exit_count = exit_count_after - exit_count_before

if exit_count <= 1
  ServiceTestHelper.print_success("Exit called at most once (idempotent)")
  ServiceTestHelper.print_info("  Exit calls: #{exit_count}")
else
